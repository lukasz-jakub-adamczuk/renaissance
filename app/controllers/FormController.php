		...
		if (isset($_POST['match']['player1']) && isset($_POST['match']['player2']) && isset($_POST['match']['vote'])) {
			if (isset($_SESSION['user']) && isset($_SESSION['user']['id'])) {
				...
				if (!empty($mYourVote)) {
					...
					// decide which phase is now
					$sCupDirectory = CACHE_DIR . '/cup';
					if (!file_exists($sCupDirectory)) {
						mkdir($sCupDirectory, 0777, true);
					}

					// all battles
					$sAllBattlesKeysFile = CACHE_DIR . '/cup/all-battles-keys';
					...
					// clear cache
					$sMatchDate = date('Y-m-d');
					$sMatchFile = CACHE_DIR . '/cup/matches/' . $sMatchDate;
					if (file_exists($sMatchFile)) {
						unlink($sMatchFile);
					}

					// clear cup battles cache
					$sAllBattlesFile = CACHE_DIR . '/cup/all-battles';
