...
class NewsInfoView extends View {
	...
	public function fill() {
		...
		// print_r($aNews);
		$this->_oRenderer->assign('aNews', $aNews);
		...
		if ($iId) {
			// title
			$this->_oRenderer->assign('sTitle', 'Squarezone - Aktualności - '.$oEntity->getField('title'));
			...
			if ($bValidNews == false) {
				...
				if ($aAssets) {
					...
					foreach ($aAssets as $ak => $asset) {
						...
						// 	// ChangeLog::add('create', $this->_sCtrlName, $mId);
						}
					}
				}
			...
			if ($aImages) {
				// print_r($aImages);
				if (count($aImages) > 1)  {
					$this->_oRenderer->assign('aFirstImage', current($aImages));
					$this->_oRenderer->assign('aImages', array_slice($aImages, 1));

					// gallery
					$this->_oRenderer->assign('aScreens', array_slice($aImages, 1));

					$aGallery['class'] = 'full';
					$aGallery['show_link'] = false;
					$aGallery['category_abbr'] = $oEntity->getField('abbr');

					$this->_oRenderer->assign('aGallery', $aGallery);
				} else {
					$this->_oRenderer->assign('aFirstImage', current($aImages));
				}
			}
			...
			$this->_oRenderer->assign('aRelatedNews', $aRelatedNews);
			...
			// comments form
			// $this->_oRenderer->assign('aCommentsForm', Comments::getFormParams('article', $oEntity));
			// $aCommentsForm = array();
			...
			$this->_oRenderer->assign('aCommentsForm', Comments::getFormParams('news', $oEntity));

			// comments
			$oCommentsCollection = Dao::collection('comments');

			$this->_oRenderer->assign('aComments', $oCommentsCollection->getCommentsById('news', $oEntity->getField('id_news')));
