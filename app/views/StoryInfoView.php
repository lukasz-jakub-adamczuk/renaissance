...
class StoryInfoView extends View {
    ...
    public function fill() {
        ...
        if ($aArticle) {
            // title
            $this->_renderer->assign('sTitle', 'Squarezone - Publicystyka - '.$oEntity->getField('category_name').' - '.$oEntity->getField('title'));
            ...
            // comments form
            $this->_renderer->assign('aCommentsForm', Comments::getFormParams('story', $oEntity));
            $this->_renderer->assign('sCommentPrimaryKey', 'id_story_comment');

            // comments
            $oCommentsCollection = Dao::collection('story-comment');

            $this->_renderer->assign('aComments', $oCommentsCollection->getCommentsById($oEntity->getField('id_story')));
