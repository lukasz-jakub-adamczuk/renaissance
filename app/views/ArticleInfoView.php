...
class ArticleInfoView extends View {
	...
	public function fill() {
		...
		if ($sSlug == 'galeria') {
			...
			if (is_array($aDirContent)) {
				if (isset($aDirContent['files'])) {
					...
					$this->_oRenderer->assign('aScreens', $aScreens);
		...
		if ($aArticle) {
			// title
			$this->_oRenderer->assign('sTitle', 'Squarezone - Gry - '.$oEntity->getField('category_name').' - '.$oEntity->getField('title'));
			...
			if (strpos($sArticleSlug, 'recenzja') !== false) {
				...
				$this->_oRenderer->assign('aRatings', $aRatings);
				...
				if (isset($aVerdict)) {
					...
					$this->_oRenderer->assign('aVerdict', $aVerdict);
				}
			}
			...
			if (isset($aFragments['logo-image'])) {
				$this->_oRenderer->assign('aLogoImage', $aFragments['logo-image']);
			}
			...
			if (isset($aFragments['game-info'])) {
				...
				$this->_oRenderer->assign('aGameInfo', $aGameInfo);
			}
			...
			if (isset($aFragments['music-info'])) {
				...
				require_once $sYamlParserPath;
					...
					$this->_oRenderer->assign('aMusicInfo', $aMusicInfo);
				// }
				// print_r($aMusicInfo);
			}
			...
			$this->_oRenderer->assign('sCommentPrimaryKey', 'id_article_comment');

			// comments
			$oCommentsCollection = Dao::collection('article-comment');

			$this->_oRenderer->assign('aComments', $oCommentsCollection->getCommentsById($oEntity->getField('id_article')));
