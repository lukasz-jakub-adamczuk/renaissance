...
class GalleryInfoView extends View {
	...
	public function fill() {
		...
		if ($aArticle) {
			// title
			$this->_oRenderer->assign('sTitle', 'Squarezone - Galerie - '.$oEntity->getField('category_name').' - '.$oEntity->getField('title'));
			...
			// comments form
			$this->_oRenderer->assign('aCommentsForm', Comments::getFormParams('gallery', $oEntity));

			// comments
			$oCommentsCollection = Dao::collection('gallery-comment');

			$this->_oRenderer->assign('aComments', $oCommentsCollection->getCommentsById($oEntity->getField('id_gallery')));
